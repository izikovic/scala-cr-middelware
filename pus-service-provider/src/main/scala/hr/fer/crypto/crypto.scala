...
object crypto {
  ...
  object io {
    ...
    def readEncodedRSANameKey(filePath: String): (Array[Byte], Array[Byte]) = {
      withDataInputStream(filePath) { stream =>
        val nameLength = stream.readInt
        val name: Array[Byte] = Array.ofDim(nameLength)
        stream.read(name)
    ...
    def readSignatureFromCert(filePath: String): (Array[Byte], Array[Byte]) = {
      withDataInputStream(filePath) { stream =>
        val keyLength = stream.readInt
        println("duzina: " + keyLength)
        val key: Array[Byte] = Array.ofDim(keyLength)
        stream.read(key)
        //stream.skip(nameLength)

        val sigLength = stream.readInt
        val signature: Array[Byte] = Array.ofDim(sigLength)
    ...
    def readEncodedRSAKey(filePath: String): Array[Byte] = {
      readEncodedRSANameKey(filePath)._2
    }

    def readEncodedRSAName(filePath: String): Array[Byte] = {
      readEncodedRSANameKey(filePath)._1
    }
